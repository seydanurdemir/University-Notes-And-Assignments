...
# Open the image file and read, encode with base 64 and remove the first character as it is a 'b'
imagePath = input('Enter the image path: ')
imageName = imagePath.split('\\')[-1]
with open(imagePath, 'rb') as file:
	img = file.read()
	img = str(base64.b64encode(img))[1:]

# Create the image command, this includes the sender, receiver, content type and MIME-Version.
# MIME allows us to attach an image to the message using a multipart message.
imgCommand = '''From: <%s>
To: <%s>
Subject: %s
MIME-Version: 1.0
Content-Type: multipart/mixed;boundary="sendImage"
--sendImage
...
%s
%s
''' % (username.decode(), destinationMail, 'CSE 476 - Lab 3', imageName, imageName, img, msg, endmsg)

# Message ends with a single period.
clientSocketSSL.send(imgCommand.encode())
